...
def tkenn_ops(x_train, y_train, x_valid, y_valid, x_test, y_test, batch_size, num_hid_layers, num_hid_nodes, af,
    ...
    dataloader = DataLoader(x_train, y_train, x_valid, y_valid, x_test, y_test, batch_size)
    DataLoader.check_data_loaders(dataloader.train_loader, x_train, y_train, num_inputs)
    DataLoader.check_data_loaders(dataloader.valid_loader, x_valid, y_valid, num_inputs)
    DataLoader.check_data_loaders(dataloader.test_loader, x_test, y_test, num_inputs)

    # Prepare TKENN architecture structure and check structure
    structure = NetworkStructure(num_hid_layers, num_hid_nodes, af, af_params, num_inputs)
    ...
    tkenn = Tkenn(device, seed, structure, weight_init=weight_init, weight_init_params=weight_init_params).double()
    tkenn_tvt = TkennTVT(loss, optimizer, init_lr, lr_scheduler, lr_scheduler_params, min_epochs, max_epochs, interval,
                         avg_interval, print_freq, log, tkenn)
    epoch_count, final_train_rmse, final_valid_rmse = tkenn_tvt.fit(device, dataloader.train_loader,
                                                                    dataloader.valid_loader, tkenn)
    y_pred, test_rmse = tkenn_tvt.perform_test(device, log, dataloader.test_loader, tkenn)
...
class DataLoader:
    ...
    def check_data_loaders(data_loader, x, y, num_inputs):
        ...
        assert data_loader.dataset.tensors[1].size(dim=0) == y.shape[0], "Error in data loader dataset shape"
        assert data_loader.dataset.tensors[1].size(dim=1) == 9, "Error in data loader dataset shape"
...
class Tkenn(nn.Module):
    def __init__(self, device, seed, structure=None, weight_init="dummy", weight_init_params="dummy"):
        ...
        self.net.add_module("output_layer", nn.Linear(self.structure.num_hid_nodes[-1], 1))
...
class TkennTVT:
    ...
    def train_one_epoch(self, train_loader, criterion, optimizer, device, model, epoch_count):
        ...
        for i, (x, y) in enumerate(train_loader):
            ...
            loss = criterion(logk_batch_pred, y)  # MSE for entire batch
    ...
    def test_rmse_ops(y_pred, test_loader, log):
        ...
        assert y_pred.shape[1] == 1, "Number of columns in y_pred does not equal number of required outputs"
