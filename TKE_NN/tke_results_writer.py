...
def write_param_txt(current_folder, folder_path, user_vars):
    ...
    var_name_list = ["trial", "num_hid_layers", "num_hid_nodes", "max_epochs", "min_epochs", "interval",
                     "avg_interval", "af", "af_params", "weight_init", "weight_init_params", "init_lr", "lr_scheduler",
                     "lr_scheduler_params", "loss", "optimizer", "batch_size", "incl_p_invars", "incl_tke_invars",
                     "incl_input_markers", "num_input_markers", "rho", "train_test_rand_split", "train_test_split_frac",
...
def write_param_csv(current_folder, folder_path, user_vars):
    ...
    var_name_list = ["trial", "num_hid_layers", "num_hid_nodes", "max_epochs", "min_epochs", "interval",
                     ...
                     "incl_input_markers", "num_input_markers", "rho", "train_test_rand_split", "train_test_split_frac",
                     "train_valid_rand_split", "train_valid_split_frac", "train_list", "valid_list", "test_list"]
    postprocess_name_list = ["run_time", "Mean_final_training_rmse", "Mean_final_validation_rmse", "Mean_testing_rmse"]
...
def write_test_truth_logk(Cx_test, Cy_test, folder_path, true_logk):

    true_logk = np.hstack((Cx_test, Cy_test, true_logk))
    np.savetxt(os.path.join(folder_path, 'LES_test_truth_logk.txt'), true_logk, delimiter=' ', header='Cx Cy log(k)')


def write_k_results(Cx_test, Cy_test, folder_path, seed, predicted_logk, current_folder):

    # Calculate k here
    predicted_k = pow(10, predicted_logk)
    to_write = np.hstack((Cx_test, Cy_test, predicted_logk, predicted_k))
    np.savetxt(os.path.join(folder_path, 'Trials', 'Trial ' + str(current_folder), 'Trial' + str(current_folder) +
                            '_seed' + str(seed) + '_TKENN_test_prediction.txt'), to_write, delimiter=' ',
